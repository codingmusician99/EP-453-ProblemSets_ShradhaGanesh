...
class MotionSynth {
    ...
    init(){
        flanger = Flanger(fmOscillator, frequency: 40, depth: 0.5, feedback: 0.5, dryWetMix: 50)
    ...
    func setFlanger(accelX: Double, accelY: Double){
        ...
        let feedback = scale(num: CGFloat(accelY), minNum: -1, maxNum: 1, scaleMin: 0, scaleMax: 1)
        
        flanger!.frequency = Float(100 + freq)
        flanger!.feedback = Float(feedback)
