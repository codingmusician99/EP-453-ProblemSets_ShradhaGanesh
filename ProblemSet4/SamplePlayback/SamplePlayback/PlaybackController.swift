...
class PlaybackController: ObservableObject {
    ...
    var piano:AudioPlayer?
    var bass:AudioPlayer?
    var loop:AudioPlayer?
    
    func setupPlayback(){
        let pianoFile = loadAudioFile(file: "Sounds/piano.wav")
        piano = AudioPlayer(file: pianoFile!)
        piano?.isLooping = false
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack{
                ...
                VStack{
                    HStack{
                        Button(action: controller.playPiano) {
                            Image("piano")
                        }
                        .padding()
